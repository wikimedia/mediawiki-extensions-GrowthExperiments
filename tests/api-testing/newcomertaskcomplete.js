...
describe( 'POST /growthexperiments/v0/newcomertask/complete', () => {
	...
	const client = new REST( 'rest.php/growthexperiments/v0/newcomertask/complete' );

	it( 'the endpoint is not accessible to anonymous users', async () => {
		const { body: sourceBody } = await client.post( '?taskTypeId=foo&revId=123' );
	...
	it( 'revId param is required', async () => {
		const { body: sourceBody } = await client.post( '?taskTypeId=foo' );
	...
	it( 'taskTypeId param is required', async () => {
		const { body: sourceBody } = await client.post( '?revId=foo' );
		assert.strictEqual( sourceBody.httpCode, 400 );
	...
	it( 'revId must be an integer', async () => {
		const { body: sourceBody } = await client.post( '?revId=foo&taskTypeId=bar' );
		assert.strictEqual( sourceBody.httpCode, 400 );
		assert.strictEqual( sourceBody.name, 'revId' );
